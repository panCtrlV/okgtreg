...
An example of command line is:
    ...
    python -u validate_backward.py 1 0 > validate-model1-seed0.out
...
on a ITAP's RCAC cluster (e.g. Radon), we need some extra work. RCAC uses PBS for job
scheduling. In order to submit parallel jobs (for example, one procedure is tested on
multiple simulated data sets), one need to prepare two bash scripts:

1. prepare a submission file which takes MODEL_ID and SEED_NUM as variables.

2. prepare a meta submission file which gives different values of MODEL_ID and SEED_NUM
   to the subission file in (1). Each pair of MODEL_ID and SEED_NUM is corresponding to
   one job to be submitted.
...
#         print("python -u validate_backward.py %d %d > validate-model%d-seed%d-%s.out" %
